...
namespace VehicleProject.Models
{
    public class Cart
    {
        ...
        public void Load(Repository<Vehicle> data)
        {
            ...
            if (storedItems?.Count > items?.Count) {
                foreach (CartItemDTO storedItem in storedItems) {
                    var book = data.Get(new QueryOptions<Vehicle> {
                        ...
                        Where = b => b.VehicleId == storedItem.VehicleId
                    });
                    if (book != null) {
        ...
        public CartItem GetById(int id) => 
            items.FirstOrDefault(ci => ci.Vehicle.VehicleId == id);

       public void Add(CartItem item) {
            var itemInCart = GetById(item.Vehicle.VehicleId);
